...
```r
initial <- create_matrix(row = 10, col = 10, start_inf = 0.1)
```

Run one basic SIR simulation:

```r
result <- simulate_sir(prob_infect = 0.25, input_matrix = initial)
result$steps
result$inf_prop
result$final_matrix
```
...
```r
simulate_many_runs(prob_infect = 0.25, input_matrix = initial, runs = 10)
```

Sweep across infection probabilities:

```r
simulate_inf_seq(input_matrix = initial, step = 0.1)
```
